...
<template>
  <div class="picross-container">
    ...
    <div v-if="gameWin" class="absolute t-0 l-0 w-full flex justify-center items-center text-xl h-full">
      GG tu as gagné mdrrr
    </div>
...
export default class Picross extends Vue {
  ...
  private exportMap() {
    ...
    for (let row = 0; row < this.solutionGrid.states.length; row++) {
      minifiedMap.push([]);
      for (const state of this.solutionGrid.states[row]) {
        minifiedMap[row].push(state === SquareState.Value ? 1 : 0);
      }
    }
    // console.log(JSON.stringify(minifiedMap));
  }
}
</script>
